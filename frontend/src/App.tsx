...
import { ScanLine, FileText, Settings2, BarChart3, Sun, Moon } from 'lucide-react';
...
function Shell() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'dark bg-[#0b0f19] text-gray-200' : 'bg-[#f8f9fc] text-gray-900'}`}>
      {/* ── Header ─────────────────────────────────────── */}
      <header className="sticky top-0 z-40 border-b border-gray-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2 shrink-0">
            ...
            Urdu OCR{' '}
            <span className="text-xs font-normal text-violet-500 bg-violet-100 dark:bg-violet-900/30 dark:text-violet-400 rounded-full px-2 py-0.5">v2</span>
          </h1>
