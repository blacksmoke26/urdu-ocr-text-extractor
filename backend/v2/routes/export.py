...
@export_router.post(
    "/export/pdf-json",
    summary="Export PDF OCR result as JSON",
    description="Export full per-page PDF OCR data with all lines aggregated.",
)
async def export_pdf_json_endpoint(result: dict):
    metrics = get_metrics()
    t0 = time.perf_counter()
    resp = {"format": "json", "data": ExportService.export_pdf_json(result)}
    metrics.api_export.record_success(files=1)
    metrics.latency_global.record((time.perf_counter() - t0) * 1000)
    metrics.total_requests.inc()
    return resp
...
@export_router.post(
    "/export/pdf-txt",
    summary="Export PDF OCR result as plain text",
    description="Extract raw text from all pages with page separators.",
)
async def export_pdf_txt_endpoint(result: dict):
    metrics = get_metrics()
    t0 = time.perf_counter()
    resp = {"format": "txt", "data": ExportService.export_pdf_txt(result)}
    metrics.api_export.record_success(files=1)
    metrics.latency_global.record((time.perf_counter() - t0) * 1000)
    metrics.total_requests.inc()
    return resp
...
@export_router.post(
    "/export/pdf-csv",
    summary="Export PDF OCR result as CSV",
    description="Convert all per-page OCR lines to CSV format with page numbers.",
)
async def export_pdf_csv_endpoint(result: dict):
    metrics = get_metrics()
    t0 = time.perf_counter()
    resp = {"format": "csv", "data": ExportService.export_pdf_csv(result)}
    metrics.api_export.record_success(files=1)
    metrics.latency_global.record((time.perf_counter() - t0) * 1000)
    metrics.total_requests.inc()
    return resp
...
@export_router.post(
    "/export/pdf-docx",
    summary="Export PDF OCR result as Word document (.docx)",
    description="Generate a formatted .docx with per-page sections and line details.",
)
async def export_pdf_docx_endpoint(result: dict):
    try:
        metrics = get_metrics()
        t0 = time.perf_counter()
        data_bytes = ExportService.export_pdf_docx(result)
        resp = {"format": "docx", "data_b64": __import__("base64").b64encode(data_bytes).decode()}
        metrics.api_export.record_success(files=1)
        metrics.latency_global.record((time.perf_counter() - t0) * 1000)
        metrics.total_requests.inc()
